...
class CargoReq:
    ...
    def load(self, cart_dev, ctrl):
        "load itself, invoke callback"
        if callable(self.onload):
            self.onload(cart_dev, self, ctrl)

    def unload(self, cart_dev, ctrl):
        "unload itself, invoke callback"
        if callable(self.onunload):
            self.onunload(cart_dev, self, ctrl)

    def just_pass_it(self, argument1=None, argument2=None):
...
class Cart:
    ...
    def finish_unloading(self, ctrl):
