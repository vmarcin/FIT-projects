...
__global__ void centerOfMass(t_particles p, float* comX, float* comY, float* comZ, float* comW, int* lock, const int N)
{
  ...
  float *wx = &partial_sums[0];
  float *wy = &partial_sums[1 * blockDim.x];
  float *wz = &partial_sums[2 * blockDim.x];
  float *w = &partial_sums[3 * blockDim.x];

  int num_threads = blockDim.x * gridDim.x;  
  int thread_id = threadIdx.x;

  // Parallel resetting of the shared memory, as we will accumulate other values into it.
  wx[thread_id] = 0.0f;
  wy[thread_id] = 0.0f;
  wz[thread_id] = 0.0f;
  w[thread_id] = 0.0f;
  ...
  for (int i = thread_id + blockIdx.x * blockDim.x; i < N; i += num_threads) {
    /**
     * Loading the weight of the particle as it will be used multiple times.
     * note:  It reduces the number of accesses to the global memory.
     */
    float weight_i = p.weight[i];

    wx[thread_id] += p.pos_x[i] * weight_i;
    wy[thread_id] += p.pos_y[i] * weight_i;
    wz[thread_id] += p.pos_z[i] * weight_i;

    w[threadIdx.x] += weight_i; 
  }

  __syncthreads(); // Wait for all threads to reduce their values

  // Reduction in shared memory
  for (int stride = blockDim.x/2; stride > 0; stride >>= 1) {
    
    wx[thread_id] += (thread_id < stride) ? wx[thread_id + stride] : 0.0f;
    wy[thread_id] += (thread_id < stride) ? wy[thread_id + stride] : 0.0f;
    wz[thread_id] += (thread_id < stride) ? wz[thread_id + stride] : 0.0f;
    w[thread_id] += (thread_id < stride) ? w[thread_id + stride] : 0.0f;      

    // All have to wait as the threads produce values for other threads (not for themselves).
    __syncthreads(); 
  }

  // Writing results into the global memory (just one thread).
  if (thread_id == 0) {
