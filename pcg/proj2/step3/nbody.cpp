                       ...
                       const int        N)
{
  ...
  #pragma acc parallel loop present(p) reduction(+:comx, comy, comz, comw)
  for (unsigned int i = 0; i < N; i++) {
    /**
    ...
    comx += p.pos_x[i] * weight;
    comy += p.pos_y[i] * weight;
    comz += p.pos_z[i] * weight;

    comw += weight;
  }
  
  return {comx/comw, comy/comw, comz/comw, comw};
