...
int main(int argc, char **argv)
{
  ...
  */
  ...
  {
    // Velocity calculation in the INTEGRATE_QUEUE
    calculate_velocity(*p_in, *p_out, N, dt);
    // Center of Mass calculation in the COM_QUEUE. 
    centerOfMassGPU(*p_in, comOnGPU, N);

    /// In step 4 - fill in the code to store Particle snapshots.
    if (writeFreq > 0 && (s % writeFreq == 0))
    {
      // Transfer particles to CPU in the MEMCPY_QUEUE
      p_in->updateHostAsync();
      // CPU waits until particles data will be available
      #pragma acc wait(MEMCPY_QUEUE)

      // Transfer centerOfMass to CPU in the MEMCPY_QUEUE
      comOnGPU.updateHostAsync();
      // while centerOfMass is copied from D2H, CPU is writing particles into output file
      h5Helper.writeParticleData(s / writeFreq);

      // CPU waits until centerOfMass data will be avilable
      #pragma acc wait(MEMCPY_QUEUE)
      h5Helper.writeCom(comOnGPU.x, comOnGPU.y, comOnGPU.z, comOnGPU.w, s / writeFreq);      
    }

    // CPU needs to wait until the centerOfMass computation ends before a new integrate step begins
    #pragma acc wait
