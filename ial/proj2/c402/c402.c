...
**/
	...
	tBTNodePtr where = NULL;		//ukazatel ktory ukazuje na miesto kde sa ma vlozit novy prvok
	bool found = false;
	//ak je koren NULL
	if (!(*RootPtr)){
		found = false;	//nenasiel uzol
		where = NULL;		
	}
	else{
		do{
			where = tmp;
			if(tmp->Cont > Content)	//ak je hodnota vkladaneho uzla mensia ideme vlavo
				tmp = tmp->LPtr;
			else if(tmp->Cont < Content) //ak je hodnota vkladaneho uzla vacsia ideme vpravo
				tmp = tmp->RPtr;
			else 
				found = true;			//nasli sme koren
		}while(!found && tmp);
	}

	//ak sme koren nenasli
	if(!found){
		tBTNodePtr newNode = (tBTNodePtr)malloc(sizeof(struct tBTNode)); //vytvorime novy uzol
		//ak sa alokacia nepodarila funckia konci
		if(!newNode)	
			return;
		newNode->Cont = Content;
		newNode->LPtr = newNode->RPtr = NULL;
		//ak where je NULL vkladame koren
		if(!where)
			*RootPtr = newNode;
		//ak je hodnota uzla kde ukazuje where vacsia ako hodnota ktoru chceme vlozit novu hodnotu vkladame vlavo
...
**/
	//ak nemame koren funkcia konci
	if(!RootPtr)
		return;
	tBTNodePtr tmp = RootPtr;
	tStackP s;	
	SInitP(&s);	//inicializacia zasobniku
	Leftmost_Preorder(tmp,&s);	//prejdeme do najlavejsieho uzla
	while(!SEmptyP(&s)){				
		tmp = STopPopP(&s);				//ziska vrchol zasobnika 	
		Leftmost_Preorder(tmp->RPtr,&s); //leftmost pre pravy podstrom
	}	
}
...
**/
	...
	tBTNodePtr tmp = RootPtr;
	tStackP s;
	SInitP(&s);	//inicializacia zasobnika
	Leftmost_Inorder(tmp,&s);		
	while(!SEmptyP(&s)){
		tmp = STopPopP(&s);		//ziskanie vrcholu
		BTWorkOut(tmp);		//spracovanie aktualneho uzlu
		Leftmost_Inorder(tmp->RPtr,&s);	//letmost pre pravy podstrom
	}
}
...
**/
	...
	tStackB sb;
	tStackP sp;
	SInitB(&sb);		//inicializacia zasobnika pre bool hodnoty
	SInitP(&sp);		//inicializacia zasobnika pre ukazatele
	Leftmost_Postorder(tmp,&sp,&sb);
	while(!SEmptyP(&sp)){
		tmp = STopPopP(&sp);		//ziskame hodntu ukazatela z vrcholu zasobnika
		SPushP(&sp,tmp);				//aktualnu hodnotu este nemozno odstranit tak ju vlozime spat na zasobnik
		left = STopPopB(&sb);		//ziskame hodnotu bool z vrcholu zasobnika
		if(left){
			SPushB(&sb,false);			
			Leftmost_Postorder(tmp->RPtr,&sp,&sb);
		}
		else{
			STopPopP(&sp);			//odstranime zo zasobnika
			BTWorkOut(tmp);			//spracujeme aktualny uzol
		}
	}	
}
...
**/
	...
	//ak rootptr je NULL funkcia nerobi nic
	if(!(*RootPtr))
		return;
	tStackP s;
	SInitP(&s); //inicializujeme zasobnik	
	tBTNodePtr tmp = NULL;
	do{
		if(!(*RootPtr)){		//ak rootptr je NULL
			if(!SEmptyP(&s))		//ak zasobnik nieje prazdny
				*RootPtr = STopPopP(&s);		//do rootptr ulozime hodnotu z vrcholu zasobniku
		}
		else{
			if(((*RootPtr)->RPtr))		//ak ma pravy podstrom
				SPushP(&s,(*RootPtr)->RPtr);	//pushneme na zasobnik
			tmp = *RootPtr;		
			*RootPtr = (*RootPtr)->LPtr;		//do rootptr dame laveho potomka
			free(tmp);
		}
	}while((!SEmptyP(&s)) || (*RootPtr));	//robime kym plati tato podmienka 
}
