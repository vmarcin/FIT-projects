...
*/
	...
	/* s->top = -1 => zasobnik je prazdny a inicializovany
	 */
	s->top = -1;  
}
...
*/
	/* vrati 1 ake je zasobik prazdny inak 0
	 */
	return (s->top == -1); 
}
...
*/
	/* vratime 1 ake je zasobnik plny inak 0
	 */
...
** funkci stackEmpty.
*/
	/* ak je zasobnik prazdny chlasime chybu a funkcia sa ukonci
	 */
	if(stackEmpty(s)){
		stackError(SERR_TOP);
		return;
	}
	/* do premennej c ulozime prvok, ktory je na vrchole zasobnika
	 */
	*c = s->arr[(s->top)];
}
...
** 
*/
	/* ak je zasobnik prazdny funkcia sa ukonci
	 */
	if(stackEmpty(s)){
		return;
	}
	/* odstranime prvok z vrcholu zasobnika
	 */
	s->top--;
}
...
** funkci stackFull.
*/
	/* ak je zasobnik plny chlasime chybu a ukoncime funkciu
	 */
	if(stackFull(s)){
		stackError(SERR_PUSH);
		return;
	}
	/* na vrchol zasobnika vlozime prvok c
	 */
	s->arr[++s->top] = c;
}
