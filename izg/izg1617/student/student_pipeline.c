...
void const* gpu_computeVertexAttributeDataPointer(
  ...
  assert(head != NULL);
	//compute pointer to attribute for a given vertex
	void const *pointer = (void*)((head->offset + (unsigned long)head->buffer + (head->stride * gl_VertexID)) * (unsigned)head->enabled);
	return pointer;
}
...
void gpu_runVertexPuller(
  ...
  assert(puller != NULL);
	for(unsigned i = 0; i < MAX_ATTRIBUTES; i++)
		//set adresses of vertex attributes
		output->attributes[i] = 
			gpu_computeVertexAttributeDataPointer(&(puller->heads[i]),gpu_computeGLVertexID(puller->indices,vertexShaderInvocation));		
}
...
void gpu_runPrimitiveAssembly(
  ...
  ///  - GPUVertexShaderInput()
	...
	GPUVertexPullerOutput vertex_puller_output;
	GPUVertexShaderInput vertex_shader_input;
	
	for(unsigned i = 0; i < nofPrimitiveVertices; i++)
	{
		gpu_runVertexPuller(&vertex_puller_output,puller,baseVertexShaderInvocation+i);
		
		vertex_shader_input.attributes = &vertex_puller_output;
		vertex_shader_input.gl_VertexID = gpu_computeGLVertexID(puller->indices,baseVertexShaderInvocation+i);
		
		vertexShader(&(primitive->vertices[i]),&vertex_shader_input,gpu);
		primitive->nofUsedVertices+=1;
	}
}
...
void gpu_computeScreenSpaceBarycentrics(
  ...
  assert(lines       != NULL);
	...
	float altitude_s;	//altitude of small triangle
	float altitude_b;	//altitude of big triangle
	for(unsigned i = 0; i < VERTICES_PER_TRIANGLE; i++)
	{
		//compute altitudes
		altitude_s = distanceTo2DLine(&lines[i],pixelCenter);
		altitude_b = distanceTo2DLine(&lines[i],&vertices[(i+2)%3]);
		//compute barycentric coordinates
		coords->data[(i+2)%3] = altitude_s / altitude_b;		
	}
}
